...
def analyze_rank_changes(teams: dict) -> None:
    ...
    complete_teams = {
        ...
        if "public_rank" in t and "private_rank" in t
    }

    # Our team
    our_team = teams.get(OUR_TEAM_ID, {})
    print(f"\nOUR TEAM (KohakuRAG, ID={OUR_TEAM_ID}):")
    print(f"  Public:  Rank #{our_team.get('public_rank', 'N/A')}, Score {our_team.get('public_score', 'N/A')}")
    print(f"  Private: Rank #{our_team.get('private_rank', 'N/A')}, Score {our_team.get('private_score', 'N/A')}")
    ...
    print("-" * 70)
    print(f"{'Rank':<6} {'TeamID':<12} {'Public':<10} {'Private':<10} {'Pub Rank':<10} {'Delta':<10}")

    private_sorted = sorted(
        complete_teams.items(),
        key=lambda x: x[1].get("private_rank", 999)
    )[:10]
    ...
    for team_id, t in private_sorted:
        ...
        print(f"#{priv_rank:<5} {team_id:<12} {pub_score:<10.3f} {priv_score:<10.3f} #{pub_rank:<9} {delta:+.3f}{marker}")

    # Top 10 in public leaderboard and their private performance
    print("\n" + "-" * 70)
    print("TOP 10 PUBLIC LEADERBOARD -> PRIVATE PERFORMANCE:")
    print("-" * 70)
    print(f"{'Pub Rank':<10} {'TeamID':<12} {'Public':<10} {'Private':<10} {'Priv Rank':<10} {'Delta':<10}")
    ...
    for team_id, t in public_sorted:
        ...
        delta = priv_score - pub_score
        marker = " <-- OURS" if team_id == OUR_TEAM_ID else ""
        print(f"#{pub_rank:<9} {team_id:<12} {pub_score:<10.3f} {priv_score:<10.3f} #{priv_rank:<9} {delta:+.3f}{marker}")
    ...
    for team_id, t, improvement in gainers[:5]:
        ...
        pub_score = t.get("public_score", 0)
        priv_score = t.get("private_score", 0)
        print(f"  #{pub_rank} -> #{priv_rank} (+{improvement} ranks): {pub_score:.3f} -> {priv_score:.3f}")
    ...
    for team_id, t, drop in losers[:5]:
        ...
        pub_score = t.get("public_score", 0)
        priv_score = t.get("private_score", 0)
        print(f"  #{pub_rank} -> #{priv_rank} (-{drop} ranks): {pub_score:.3f} -> {priv_score:.3f}")
...
def generate_latex_table(teams: dict) -> str:
    ...
    lines.append("\\caption{Final leaderboard comparison showing public vs.\\ private score dynamics. Only KohakuRAG maintained the top position across both partitions.}")
    lines.append("\\label{tab:leaderboard_comparison}")
    lines.append("\\small")
    lines.append("\\begin{tabular}{lccc}")
    lines.append("\\toprule")
    lines.append("\\textbf{Team} & \\textbf{Public} & \\textbf{Private} & \\textbf{$\\Delta$} \\\\")
    ...
    lines.append(f"KohakuRAG (Ours) & {our['public_score']:.3f} (\\#1) & {our['private_score']:.3f} (\\#1) & ${delta:+.3f}$ \\\\")
    ...
    for team_id, t in private_sorted[1:3]:  # #2 and #3
        ...
        delta = priv_score - pub_score
        lines.append(f"Private \\#{priv_rank} & {pub_score:.3f} (\\#{pub_rank}) & {priv_score:.3f} (\\#{priv_rank}) & ${delta:+.3f}$ \\\\")
    ...
    for team_id, t in public_sorted[:2]:  # Top 2 excluding us
        ...
        priv_rank = t["private_rank"]
        delta = priv_score - pub_score
        lines.append(f"Public \\#{pub_rank} & {pub_score:.3f} (\\#{pub_rank}) & {priv_score:.3f} (\\#{priv_rank}) & ${delta:+.3f}$ \\\\")
